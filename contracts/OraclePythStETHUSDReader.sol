...
/// @title OraclePythStETHUSDReader
/// @notice A contract that reads stETH/USD price data using Pyth oracles
/// @dev Implements a Chainlink-compatible interface using two Pyth oracles
contract OraclePythStETHUSDReader {
    /// @notice The Pyth oracle for ETH/USD price
    PythAggregatorV3 public immutable oracleETHUSD;

    /// @notice The Pyth oracle for stETH/ETH price
    PythAggregatorV3 public immutable oracleStETHETH;
    ...
    /// @param _oracleETHUSD The address of the ETH/USD Pyth oracle
    /// @param _oracleStETHETH The address of the stETH/ETH Pyth oracle
    constructor(address _oracleETHUSD, address _oracleStETHETH) {
        oracleETHUSD = PythAggregatorV3(_oracleETHUSD);
        oracleStETHETH = PythAggregatorV3(_oracleStETHETH);
    }

    /// @notice Gets the latest stETH/USD price data in a Chainlink-compatible format
    /// @return roundId The timestamp of the latest update
    /// @return answer The latest price value of stETH/USD
    ...
    {
        // Get ETH/USD price data
        (
            ,
            // uint80 ethRoundId (unused)
            int256 ethAnswer, // uint256 ethStartedAt (unused)
            ,
            uint256 ethUpdatedAt, // uint80 ethAnsweredInRound (unused)

        ) = oracleETHUSD.latestRoundData();

        // Get stETH/ETH price data
        (
            ,
            // uint80 stETHRoundId (unused)
            int256 stETHAnswer, // uint256 stETHStartedAt (unused)
            ,
            uint256 stETHUpdatedAt, // uint80 stETHAnsweredInRound (unused)

        ) = oracleStETHETH.latestRoundData();

        // Use the latest timestamp between the two oracles
        uint256 timestamp = ethUpdatedAt >= stETHUpdatedAt
            ? ethUpdatedAt
            : stETHUpdatedAt;

        // Calculate stETH/USD price
        // Need to handle decimals correctly
        uint8 ethDecimals = oracleETHUSD.decimals();
        uint8 stETHDecimals = oracleStETHETH.decimals();

        // Calculate stETH/USD = stETH/ETH * ETH/USD, adjusting for decimal differences
        int256 stETHUSDPrice = (stETHAnswer * ethAnswer) /
            int256(10 ** uint256(ethDecimals + stETHDecimals - DECIMALS));
    ...
    function description() public pure returns (string memory) {
        return "Pyth Oracle for stETH/USD price";
    }
}