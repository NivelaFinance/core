...
contract MiscHelper is Ownable, ReentrancyGuard {
    ...
    /// @notice Staked nUSD token contract
    IStakednUSD public immutable stnUSD;
    ...
    constructor(IBaseContracts baseContracts_) {
        ...
        _ensureNonzeroAddress(address(stnUSD));
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Deposits into staked nUSD
     * @param amount Deposit amount
     * @dev Includes reentrancy protection
     *
     * Requirements:
     ...
     *
     * Effects:
     * · Deposits into staked nUSD
     * · Updates interest rates
     */
    function depositStakednUSD(uint256 amount) external nonReentrant {
        _distributeFees();
        stnUSD.deposit(_msgSender(), _msgSender(), amount);
        _updateInterestRateAndBorrowLimit();
    }

    /**
     * @notice Withdraws from staked nUSD
     * @dev Includes reentrancy protection
     *
     * Requirements:
     ...
     *
     * Effects:
     * · Withdraws from staked nUSD
     * · Updates interest rates
     */
    function withdrawStakednUSD() external nonReentrant {
        _distributeFees();
        stnUSD.withdraw(_msgSender());
        _updateInterestRateAndBorrowLimit();
    }
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Swaps nUSD for stable using PSM
     * @param psm PSM contract
     * @param receiver Receiver address
     * @param amountnUSD nUSD amount
     * @dev Includes reentrancy protection
     *
     * Requirements:
     * · Valid PSM contract
    ...
    function psmSwapnUSDForStable(
        ...
        psm.swapnUSDForStable(_msgSender(), receiver, amountnUSD);
        _updateInterestRateAndBorrowLimit();
    }

    /**
     * @notice Swaps stable for nUSD using PSM
     * @param psm PSM contract
     * @param receiver Receiver address
    ...
    function psmSwapStableFornUSD(
        ...
        psm.swapStableFornUSD(_msgSender(), receiver, amountStable);
        _updateInterestRateAndBorrowLimit();
    }
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Calculates APR for staked nUSD
     * @return APR value
     * @dev Includes reentrancy protection
     *
     * Requirements:
     * · Valid staked nUSD balance
     *
     * Effects:
     * · Calculates APR
     */
    function aprStakednUSD() external view returns (uint256) {
        ...
        if (IERC20Custom(address(stnUSD)).totalSupply() == 0) {
            return 0;
        } else {
            return
                ((totalBorrowed *
                    ...
                    TENK_PRECISION) /
                IERC20Custom(address(stnUSD)).totalSupply();
        }
    }
    ...
    function _updateInterestRateAndBorrowLimit() private {
        ...
        for (uint256 i; i < pegStabilityModulesLength; i++) {
            psmTotalCapital += pegStabilityModules[i].nusdMinted();
        }
