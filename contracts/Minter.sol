...
contract Minter is Ownable, ReentrancyGuard {
    ...
    constructor(IBaseContracts baseContracts_, address treasury_) {
        ...
        maxMintAmount = ((73_146_000 * 25) / 10) * 10 ** niv.decimals();
    ...
    function mintRemainingSupply(
        ...
        if (!publicMintActive) revert MintNotActive();
        if (tokenReserve() >= niv.maxSupply() / 100)
            revert TokenReserveIsNotLowEnough();
        uint256 decimalsDifference = 10 **
            (niv.decimals() - stablecoin.decimals());
     ...
     * @notice Sends accumulated nUSD to floor contract
     */
    function sendToFloornUSD() external nonReentrant {
        _sendToFloornUSD();
    }
    ...
    function calcMintAmount(
        ...
        for (uint256 i; i < RATES_LENGTH; i++) {
            supplyRangeLeft[i] = (niv.maxSupply() / RATES_LENGTH);
            // Check for overflow in maxStablePerRange calculation
            uint256 maxStable = ((niv.maxSupply() / RATES_LENGTH) / rates[i]) /
                decimalsDifference;
            if (maxStable == 0 && (niv.maxSupply() / RATES_LENGTH) != 0) {
                revert MaxStableRangeCalculationFailed(
                    niv.maxSupply() / RATES_LENGTH,
                    rates[i]
                );
            }
    ...
    function calcStableAmount(
        ...
        for (uint256 i; i < RATES_LENGTH; i++) {
            maxStablePerRange[i] =
                ((niv.maxSupply() / RATES_LENGTH) / rates[i]) /
                decimalsDifference;
        }
        for (uint256 i; i < RATES_LENGTH; i++) {
            if (currentRatio < ((i + 1) * 100) / RATES_LENGTH) {
    ...
    function tokenReserve() public view returns (uint256) {
        return niv.maxSupply() - supplyMinted;
    }
    ...
    function _mint(IERC20Custom stablecoin, uint256 stableAmount) private {
        ...
        uint256 floorAmount = (stableAmount * (100 - TREASURY_ALLOCATION)) /
            100;
        supplyMinted += toMint;
        currentRatio = (supplyMinted * 100) / niv.maxSupply();
        ...
        IERC20TokenRebase(address(niv)).rebase();
        IERC20TokenRebase(address(niv)).mint(_msgSender(), toMint);
        uint256 length = stables.length;
        for (uint256 i; i < length; i++) {
            if (stablecoin == stables[i]) {
                helper.psmSwapStableFornUSD(
    ...
    function _sendToFloornUSD() private {
        uint256 nusdBalance = nusd.balanceOf(address(this));
        if (nusdBalance > 0) {
            helper.depositFloor(nusdBalance);
        }
    }
    ...
    function _setupApprovals(
        ...
        bool successnUSD = nusd.approve(address(floor), type(uint256).max);
        if (!successnUSD) {
            revert ApproveFailed();
        }
    }
