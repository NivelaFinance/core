...
contract SupplyHangingCalculator is Ownable, ISupplyHangingCalculator {
    ...
    /// @notice Staked nUSD contract reference
     ...
     *
     * Calculation Steps:
     * 1. Get total nUSD supply
     * 2. Subtract vault balance
     * 3. Apply safety margin to floor capital
     * 4. Apply safety margin to staked nUSD supply
     * 5. Compute final supply hanging
     */
    function getSupplyHanging() external view returns (uint256) {
        uint256 supplynUSD = nusd.totalSupply();
        uint256 vaultBalance = nusd.balanceOf(address(vault));
        uint256 adjustedSupplynUSD = supplynUSD - vaultBalance;
        // Apply safety margin to floor capital
        uint256 capitalFloor = (floor.capital() * (100 - safetyMargin)) / 100;
        // Apply safety margin to staked nUSD supply
        uint256 supplyStakednUSD = (IERC20Custom(address(stnUSD))
            .totalSupply() * (100 - safetyMargin)) / 100;
        return adjustedSupplynUSD - capitalFloor - supplyStakednUSD;
    }
    ...
    function getSupplyHangingUnsafe() external view returns (uint256) {
        ...
        uint256 supplyStakednUSD = IERC20Custom(address(stnUSD)).totalSupply();
        return adjustedSupplynUSD - capitalFloor - supplyStakednUSD;
    }
