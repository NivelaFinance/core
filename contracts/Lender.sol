...
contract Lender is Ownable, ReentrancyGuard {
    ...
    InUSDProvider public immutable nusdProvider;
    ...
    constructor(
        ...
        nusdProvider = baseContracts_.nusdProvider();
        ...
        _ensureNonzeroAddress(address(nusdProvider));
        ...
        IOracle oracle_;
        if (address(collateral_) == address(baseContracts_.veNIV())) {
        ...
        returns (uint256 part, uint256 share)
    {
        ...
        uint256 maxAmount = vault.toAmount(
            nusd,
            vault.balanceOf(nusd, msgSender),
            false
        );
        amount = maxAmount > amount ? amount : maxAmount;
        vault.withdraw(nusd, msgSender, msgSender, amount, 0);
    }
    ...
    function repay(
        ...
        uint256 share = vault.toShare(nusd, amount, true);
        vault.transfer(
            nusd,
    ...
    function liquidate(
        ...
        uint256 allBorrowShare = vault.toShare(nusd, allBorrowAmount, true);
        vault.transfer(nusd, liquidator, address(this), allBorrowShare);
    ...
    function withdrawFees(uint256 amountToProvide) external nonReentrant {
        ...
        uint256 share = vault.toShare(nusd, _feesEarned, false);
        accrueInfo.feesEarned = 0;
        nusdProvider.provide(amountToProvide);
        vault.transfer(nusd, address(this), feeTo, share);
    ...
    function _borrow(
        ...
        uint256 lendernUSDAmount = vault.toAmount(
            nusd,
            vault.balanceOf(nusd, address(this)),
            false
        );
        uint256 amountToProvide;
        if (amount > lendernUSDAmount) {
            amountToProvide = amount - lendernUSDAmount;
        } else {
            amountToProvide = 0;
        }
        nusdProvider.provide(amountToProvide);
        share = vault.toShare(nusd, amount, false);
        vault.transfer(nusd, address(this), to, share);
