 ...
 * @dev Oracle implementation for calculating the floor price of veNIV tokens
 * @notice Provides floor price calculations based on the total capital in the Floor contract
 * and the total rebased supply of NIV tokens. The floor price represents the minimum
 * value of veNIV tokens backed by the capital in the Floor contract.
 */
contract OracleFloorPrice {
    ...
    /// @notice NIV token contract for supply calculations
    IERC20TokenRebase public immutable niv;
    /// @notice veNIV contract for price queries
    IVoteEscrowedNIV public immutable veNIV;
    ...
    error ZeroAddress();
    /// @notice Thrown when querying price for non-veNIV asset
    error WrongAddress();
    ...
    constructor(IBaseContracts baseContracts_) {
        ...
        niv = baseContracts_.niv();
        veNIV = baseContracts_.veNIV();
        floor = baseContracts_.floor();
        // Validate addresses
        _ensureNonzeroAddress(address(niv));
        _ensureNonzeroAddress(address(veNIV));
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Calculates the current floor price of veNIV
     * @param asset Address of the asset to price (must be veNIV)
     * @return price Current floor price with round precision
     * @dev Floor price is calculated as: (total capital / total NIV supply)
     */
    function getPrice(address asset) public view virtual returns (uint256) {
        if (asset != address(veNIV)) revert WrongAddress();
        uint256 floorCapital = floor.capital();
        if (floorCapital == 0) return 0;
        uint256 totalSupply = niv.totalSupplyRebased() == 0
            ? niv.totalSupply()
            : niv.totalSupplyRebased();
