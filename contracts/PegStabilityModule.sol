 ...
 * @notice Manages nUSD-stablecoin swaps
 */
contract PegStabilityModule is Ownable, ReentrancyGuard, IPSM {
    ...
    enum Direction {
        IN, // Swapping stablecoins for nUSD
        OUT // Swapping nUSD for stablecoins
    }
    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    ...
    /// @notice Maximum amount of nUSD that can be minted
    /// @dev Prevents excessive token creation
    uint256 public nusdMintCap;
    /// @notice Current amount of nUSD minted through this module
    /// @dev Tracks minted tokens for cap enforcement
    uint256 public nusdMinted;
    ...
    /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/
    /// @notice Emitted when nUSD mint cap is changed
    event nUSDMintCapChanged(uint256 oldCap, uint256 newCap);
    /// @notice Emitted when stable tokens are swapped for nUSD
    event StableFornUSDSwapped(uint256 stableIn, uint256 mintednUSD);
    /// @notice Emitted when nUSD is swapped for stable tokens
    event nUSDForStableSwapped(uint256 burnednUSD, uint256 stableOut);
    ...
    error ZeroAmount();
    /// @notice Thrown when nUSD mint cap would be exceeded
    error nUSDMintCapReached();
    /*//////////////////////////////////////////////////////////////
                            MODIFIERS
    //////////////////////////////////////////////////////////////*/
    ...
    function swapnUSDForStable(
        ...
        _ensureNonzeroAmount(amountnUSD);
        _ensureNonzeroAddress(treasury);
        // Check if there's enough nUSD minted to burn
        if (nusdMinted < amountnUSD) {
            revert InvalidAmount();
        }
        // Burn nUSD first to prevent reentrancy
        unchecked {
            nusdMinted -= amountnUSD;
        }
        stableOwner.burn(msgSender, amountnUSD);
        ...
        uint256 remainingStable = MathLib.subWithZeroFloor(
            (amountnUSD * 10 ** stableToken.decimals()) / MANTISSA_ONE,
            stableTokenAmountUSD
        );
        if (
            remainingStable > 0 &&
        ...
        emit nUSDForStableSwapped(amountnUSD, stableTokenAmountUSD);
    }
    function SwapsnUSDforNative(
        IERC20 _token,
        uint256 _amount,
        address _to
    ) external onlyOwner {
    ...
    /**
     * @notice Swaps stable tokens for nUSD
     * @param msgSender Original swap initiator
     * @param receiver nUSD token recipient
     * @param amountStable Stable token amount to swap
     *
     * Requirements:
     * · Only callable by helper
     * · Non-zero receiver address
     * · Within nUSD mint cap
     *
     * Effects:
     * · Transfers stable tokens
     * · Mints nUSD tokens
     * · Updates oracle price
     * · Emits StableFornUSDSwapped event
     */
    function swapStableFornUSD(
        address msgSender,
        address receiver,
        ...
        if (nusdMinted + actualTransferAmtInUSD > nusdMintCap) {
            revert nUSDMintCapReached();
        }
        unchecked {
            nusdMinted += actualTransferAmtInUSD;
        }
        ...
        emit StableFornUSDSwapped(actualTransferAmt, actualTransferAmtInUSD);
    }

    /**
     * @notice Sets the maximum amount of nUSD that can be minted through this contract
     * @param nusdMintCap_ The new maximum amount of nUSD that can be minted
     */
    function setnUSDMintCap(uint256 nusdMintCap_) external onlyOwner {
        uint256 oldnUSDMintCap = nusdMintCap;
        nusdMintCap = nusdMintCap_;
        emit nUSDMintCapChanged(oldnUSDMintCap, nusdMintCap_);
    }
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Preview the amount of stable tokens received for burning nUSD
     * @param amountnUSD Amount of nUSD to hypothetically burn
     * @return Amount of stable tokens that would be received
     */
    function previewSwapnUSDForStable(
        uint256 amountnUSD
    ) external view returns (uint256) {
        _ensureNonzeroAmount(amountnUSD);
        uint256 stableTokenAmountUSD = _previewTokenUSDAmount(
    ...
    /**
     * @notice Preview the amount of nUSD received for depositing stable tokens
     * @param amountStable Amount of stable tokens to hypothetically deposit
     * @return Amount of nUSD that would be received
     */
    function previewSwapStableFornUSD(
        uint256 amountStable
    ) external view returns (uint256) {
        _ensureNonzeroAmount(amountStable);
        uint256 nusdTokenAmountUSD = _previewTokenUSDAmount(
            amountStable,
            Direction.IN
        );
        if (nusdMinted + nusdTokenAmountUSD > nusdMintCap) {
            revert nUSDMintCapReached();
        }
        return nusdTokenAmountUSD;
    }
