...
contract BaseContracts is Ownable {
    ...
    InUSDProvider private _nusdProvider;
    ...
    function setnUSD(IERC20Token nusd_) external onlyOwner {
        _checkZeroAddress(address(nusd_));
        _checkContractNotSet(address(_coreTokens.nusd));
        _coreTokens.nusd = nusd_;
    }

    function setnUSDProvider(InUSDProvider provider_) external onlyOwner {
        _checkZeroAddress(address(provider_));
        _checkContractNotSet(address(_nusdProvider));
        _nusdProvider = provider_;
    }
    ...
    function setStakednUSD(IStakednUSD stnUSD_) external onlyOwner {
        _checkZeroAddress(address(stnUSD_));
        _checkContractNotSet(address(_coreTokens.stnUSD));
        _coreTokens.stnUSD = stnUSD_;
    }
    ...
    function setNIV(IERC20TokenRebase niv_) external onlyOwner {
        _checkZeroAddress(address(niv_));
        _checkContractNotSet(address(_coreTokens.niv));
        _coreTokens.niv = niv_;
    }
    ...
    function setVoteEscrowedNIV(IVoteEscrowedNIV veNIV_) external onlyOwner {
        _checkZeroAddress(address(veNIV_));
        _checkContractNotSet(address(_coreTokens.veNIV));
        _coreTokens.veNIV = veNIV_;
    }

    // Convenience getters for struct members
    function nusd() external view returns (IERC20Token) {
        return _coreTokens.nusd;
    }

    function niv() external view returns (IERC20TokenRebase) {
        return _coreTokens.niv;
    }

    function stnUSD() external view returns (IStakednUSD) {
        return _coreTokens.stnUSD;
    }

    function veNIV() external view returns (IVoteEscrowedNIV) {
        return _coreTokens.veNIV;
    }
