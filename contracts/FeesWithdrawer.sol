...
contract FeesWithdrawer is Ownable, IFeesWithdrawer {
    ...
    /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/
    /// @notice Emitted when total nUSD is withdrawn
    /// @param amount Total withdrawn amount
    event nUSDTotalWithdrawn(uint256 amount);
    /// @notice Emitted per-vault withdrawal
    /// @param vault Vault contract address
    /// @param amount Withdrawn amount
    event nUSDWithdrawn(IVault indexed vault, uint256 amount);
    /*//////////////////////////////////////////////////////////////
                            CUSTOM ERRORS
    //////////////////////////////////////////////////////////////*/
    ...
    function withdraw() external onlyOwner {
        ...
        for (uint256 i; i < length; i++) {
            ...
            uint256 lendernUSDAmount = vault.toAmount(
                nusd,
                vault.balanceOf(nusd, address(lender)),
                false
            );
            uint256 amountToProvide;
            if (feesEarned > lendernUSDAmount) {
                amountToProvide = feesEarned - lendernUSDAmount;
            } else {
                amountToProvide = 0;
            }
            lender.withdrawFees(amountToProvide);
            address vaultAddress = address(lender.vault());
            uint256 vaultnUSDBalance = vault.balanceOf(nusd, vaultAddress);
            vault.transfer(nusd, vaultAddress, address(this), vaultnUSDBalance);
        }
        uint256 totalAmount = 0;
        uint256 share = vault.balanceOf(nusd, address(this));
        if (share != 0) {
            ...
            totalAmount += amount;
            emit nUSDWithdrawn(vault, amount);
        }
        emit nUSDTotalWithdrawn(totalAmount);
    }
